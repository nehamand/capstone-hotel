...
describe('DELETE /hiredservices', () => {
  ...
  test('Should be able to disable a hired service', async () => {
    const response = await request(app)
      .delete(`/hiredservices/${hiredService.id}`)
      .set({ Authorization: `Bearer ${token}` })

    expect(response.status).toEqual(200)
    expect(response.body.message).toEqual("Hired service disabled")
  })
})

describe('Errors tests on DELETE /hiredservices', () => {
  let connection: DataSource
  ...
  test('Should not be able to disable a hired service without admin permission', async () => {
    const response = await request(app)
      .delete(`/hiredservices/${hiredService.id}`)
